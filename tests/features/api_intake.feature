Feature: Intake API
    ...
    Scenario: Get list of all unrecognized and unscanned files
        Given user "<user>" is authenticated
        And the Yoda intake API is queried for all unrecognized and unscanned files for collection "<coll>"
        Then the response status code is "200"
        And ...
    ...
    Scenario: Get list of all datasets
        Given user "<user>" is authenticated
        And the Yoda intake API is queried for all intake datasets for collection "<coll>"
        Then the response status code is "200"
        And ...
    ...
    Scenario: Scan for and recognize datasets in study intake area
        Given user "<user>" is authenticated
        And the Yoda intake API is to scan for all intake datasets for collection "<coll>"
        Then the response status code is "200"
        And ...
    ...
    Scenario: Lock dataset in study intake area
        ...
        And the Yoda intake API is to lock dataset id and collection "<coll>"
        Then the response status code is "200"
        And ...
    ...
    Scenario: Unlock dataset in study intake area
        Given user "<user>" is authenticated
        And dataset id is retrieved from dataset list
        And the Yoda intake API is to unlock datasets for path "<path>"
        Then the response status code is "200"
        And ...
